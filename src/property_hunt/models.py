...
class Listing(StrictModel):
    ...
    canonical_building_id: str | None = None
    canonicalization_confidence: float | None = Field(None, ge=0, le=1)
    canonicalization_reasons: list[str] = Field(default_factory=list)
    layout_group_id: str | None = None
    layout_confidence: float | None = Field(None, ge=0, le=1)
    layout_signals: list[str] = Field(default_factory=list)
    duplicate_group_id: str | None = None
    duplicate_confidence: float | None = Field(None, ge=0, le=1)
...
class Transaction(StrictModel):
    ...
    def price_per_sqft(self) -> float:
        return self.price_aed / self.area_sqft
...
class RentalRecord(StrictModel):
    ...
    building_name: str
    community: str | None = None
    annual_rent_aed: float = Field(gt=0)
...
class Project(StrictModel):
    ...
    advertised_end_date: date | None = None
    completion_percent: float | None = Field(None, ge=0, le=100)
...
class AuctionRecord(StrictModel):
    id: str
    source: str = "emirates_auction"
    title: str
    url: str | None = None
    location: str | None = None
    property_type: str | None = None
    auction_date: date | None = None
    current_bid_aed: float | None = Field(None, ge=0)
    reserve_aed: float | None = Field(None, ge=0)
    area_sqft: float | None = Field(None, gt=0)
    bedrooms: int | None = Field(None, ge=0)
    status: str = "active"
    provenance: Provenance
...
class MarketMetric(StrictModel):
    id: str
    source: str
    period: str
    segment: str
    transaction_count: int | None = Field(None, ge=0)
    transaction_value_aed: float | None = Field(None, ge=0)
    median_price_psf: float | None = Field(None, ge=0)
    provenance: Provenance


class ServiceChargeRecord(StrictModel):
    id: str
    project_name: str
    usage: str | None = None
    year: int
    rate_aed_sqft: float = Field(ge=0)
    provenance: Provenance


class RegistryEntity(StrictModel):
    id: str
    entity_type: str
    name: str
